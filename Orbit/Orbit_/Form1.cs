...
namespace Orbit_
{
    public partial class Form1 : Form
    {
        ...
        public struct SysVars
        {
            ...
            public int iMushD;
            public SysVars(double fRandForce, double fOPInc, double fEarthMass, int iMushD)
            {
        ...
        private void LoadMushImage()
        {
            ...
            SourceImage = new Bitmap(img);
            Rectangle CropRect;
            CropRect = new Rectangle(0, 0, mySysVars.iMushD, mySysVars.iMushD);
            Image croppedImage;
            croppedImage = SourceImage.Clone(CropRect, SourceImage.PixelFormat);
            TB = new TextureBrush(croppedImage);

            bmMush = new Bitmap(mySysVars.iMushD, mySysVars.iMushD);
            Graphics G = Graphics.FromImage(bmMush);
            //Draw our cropped image onto the output image as an ellipse with the same width/height (circle)
        ...
        private void pbGalaxy_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
        {
            ...
            for (int i = 0; i < myBalls.Count; i++)
            {
                ...
                {
                    if (myBalls[i].b.bBadPos && Properties.Settings.Default.bExplosion)
                    {
                        if (bExVisible)
                        {
                            int iOffsetX = 0;
                            int iOffsetY = 0;
                            int iDistT = 0;
                            iXStart = (int)((myBalls[i].b.fPos[0]) + (pbGalaxy.Width - mySysVars.iMushD) / 2);
                            iYStart = (int)((-myBalls[i].b.fPos[1]) + (pbGalaxy.Height - mySysVars.iMushD) / 2);
                            ...
                            iDistT = (int)(Math.Sqrt(Math.Pow(myBalls[i].b.fPos[0], 2) + Math.Pow(myBalls[i].b.fPos[1], 2)));

                            iOffsetX = (int)((myBalls[i].b.fPos[0] / iDistT) * (mySysVars.iMushD / 2));
                            iOffsetY = -(int)((myBalls[i].b.fPos[1] / iDistT) * (mySysVars.iMushD / 2));

                            bmMushRot = RotateImg(bmMush, dAngle * 180 / Math.PI, Color.Transparent);
                            g.DrawImage(bmMushRot, iXStart + iOffsetX, iYStart + iOffsetY);
                        }
                    }
                    else
                    {
                        ...
                        g.FillEllipse(b, (int)((myBalls[i].b.fPos[0]) + (pbGalaxy.Width - myBalls[i].b.fDiam) / 2), (int)((-myBalls[i].b.fPos[1]) + (pbGalaxy.Height - myBalls[i].b.fDiam) / 2), (int)myBalls[i].b.fDiam, (int)myBalls[i].b.fDiam);
                    }
                 }
            }
